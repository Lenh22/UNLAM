function validar(){
    ...
    if ($("#nombre").val()==""){ /*valida que nombre no este vacio*/
        ...
        $("#nombre").after("<p class='error'>El nombre es obligatorio</p>")
    }
    if($("#apellido").val().length < 5){
        $("#apellido").after("<p class='error'>El campo apellido debe tener al menos 4 caracteres </p>");
        error++;
...
/*--REPASO Manipulacion de DOM con jquery--
    <h1></h1>
    $("selector").append() //agrega
    $("selector").empty() //elimina el contenido pero no las etiquetas
    $("selector").text() //pisa el contenido
    $("selector").detach()//elimina lo que seleccioné
    $("selector").remove() //elimina lo que seleccioné
    $("selector").before()//agrega antes
    $("selector").after()//agrega despues
